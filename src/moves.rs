...
mod tests {
    ...
    pub fn test_inner(fen: &str, max_depth: usize, expected: &[u64]) {
        let board = Board::from_fen(fen).unwrap();
        for depth in 1..=max_depth {
            assert_eq!(board.perft(depth), expected[depth - 1]);
        }
    }

    // TODO: Probably make this a proc macro so we can split each depth into a separate test
    //
        ...
        ($({$name:ident, $fen:expr, $expected:tt}),*) => {
            $(
                mod $name {
                    use super::*;
                    #[test]
                    fn perft() { test_inner($fen, $expected.len(), &$expected); }
                }
            )*
        }
    }
